...
export function WhyBungomaSection({ cmsData }) {
  const whyUsData = cmsData?.whyUs || {};
  const items = whyUsData.items?.length > 0 ? whyUsData.items : DEFAULT_WHY_ITEMS;
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-14">
          <p className="text-savanna-400 font-semibold uppercase tracking-widest text-sm mb-3">
            {whyUsData.subtitle || "✦ Why Choose Us"}
          </p>
          <h2 className="font-display text-4xl md:text-5xl font-bold text-white mb-4">
            {whyUsData.titleLine1 || "The Bungoma Tours "}
            <span className="text-gradient-terracotta">{whyUsData.titleLine2 || "Difference"}</span>
          </h2>
          <p className="text-white/60 max-w-xl mx-auto">
            {whyUsData.description || "We go beyond ordinary tourism to give you genuine, deeply-rooted African experiences that stay with you forever."}
          </p>
        </div>
        ...
        <div className="text-center mt-14">
          ...
          >
            {whyUsData.cta || "Start Your Journey"}
          </a>
        </div>
      </div>
    </section>
